...
import bin.modelo.FacturaAlojamiento;
import bin.modelo.FormaPago;
				...
				if (opcion_seleccionada == 1) {
						...
						try {
							...
							if (opcion_Recepcionosta == 1) {
								...
								if(hotelito.getCapacidadMaxAdultos()>(hotelito.getCapacidadActualAdultos()+huesped.getCantidadAdultos())&&hotelito.getCapacidadMaxNiños()>(hotelito.getCapacidadActualNiños()+huesped.getCantidadInfantes())){
									...
									//CrearFactura por alojamiento;
									int diasEstancia=Integer.parseInt(input("cuantas noches se va a quedar el grupo: "));
									int totalPorNoche=0;
									Map<String, Habitacion> habitacionesHotel= hotelito.getHabitaciones();
									for (int s=0; s<habitacionesHuesped.size();s++) {
										Habitacion habitacion= habitacionesHotel.get(s);
										String tipo=habitacion.getTipoHabitacion();
										if (tipo.equals("Estandar")) {
											totalPorNoche+=Tarifa.precioEstandar;
										}
										else if(tipo.equals("Suite")) {
											totalPorNoche+=Tarifa.precioSuit;
										}
										else if(tipo.equals("DoubleSuite")) {
											totalPorNoche+=Tarifa.precioDoubleSuite;
										}
									}
									int totalAlojamiento=totalPorNoche*diasEstancia;
									FacturaAlojamiento facturaAlojamiento= new FacturaAlojamiento(diasEstancia, totalPorNoche, totalAlojamiento);
									huesped.setFacturaAlojamiento(facturaAlojamiento);
							...
							String anfitrion=input("ingrese el nombre del anfitrion cuyo grupo va a hacer el check-out");
							Map<String,Huesped> grupos=hotelito.getGrupos();
							Huesped grupo=grupos.get(anfitrion);
							//Desocupar las habitaciones//
							ArrayList<String> idsHabitaciones= grupo.getIdentificadoresHabitaciones();
							Map<String,Habitacion> habitaciones=hotelito.getHabitaciones();
							
							for(int i=0;i<idsHabitaciones.size();i++) {
								String id= idsHabitaciones.get(i);
								Habitacion habitacion=habitaciones.get(id);
								habitacion.setOcupada(false);
							}
							ArrayList<Factura> gastos=grupo.getConsumosGrupo();
							for (int a=0; a<gastos.size();a++) {
								Factura factura=gastos.get(a);
								FormaPago formaPago= factura.getFormaPago();
								if (formaPago.equals(FormaPago.cargoGrupo)){
									String pagar=String.valueOf(factura.getTotal());
									System.out.println("El anfitrion tiene una deuda de: " + pagar);
								}
							FacturaAlojamiento facturaAlojamiento = grupo.getFacturaAlojamiento();
							String pagarAlojamiento= String.valueOf(facturaAlojamiento.getTotalAlojamiento());
							System.out.println("El anfition tiene una deuda de: "+ pagarAlojamiento);
							}
					...
					while (continuar3) {
						try {
							...
							TipoCobro tipo= servicioSeleccionado.getTipoCobro();
							if (tipo.equals(TipoCobro.persona)) {
								double multiplicador=Double.parseDouble(input("Igrese el numero de personas"));
								precioServicio= precioServicio*multiplicador;
							}
							...
							int masServicios= Integer.parseInt(input("desea agregar mas servicios: \n 1. si \n 2. no" ));
							if (masServicios==2) {
								termino=true;
							}
							}
							int formaPago= Integer.parseInt(input("ingrese el tipo de pago: \n 1. inmediato \n 2. cargo al grupo"));
							if (formaPago==1) {
								nuevafactura.setFormaPago(FormaPago.inmediato);
							}
							else if(formaPago==2) {
								nuevafactura.setFormaPago(FormaPago.cargoGrupo);
							}
							else {
								System.out.println("opción no valida se asignara cargo al grupo");
								nuevafactura.setFormaPago(FormaPago.cargoGrupo);
							}
							String anfitrion= input("ingrese el nombre del huesped anfitrion");
							Huesped grupo= hotelito.getGrupos().get(anfitrion);
							ArrayList<Factura> facturas=grupo.getConsumosGrupo();
							facturas.add(nuevafactura);
							grupo.setConsumosGrupo(facturas);
							}
